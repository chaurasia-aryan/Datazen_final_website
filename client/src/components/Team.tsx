              ...
              <div className="relative">
                ...
                >
                  {category.members.map((member, memberIndex) => (
                    <motion.div
                      key={memberIndex}
                      className="flex-shrink-0 w-56 rounded-xl bg-white border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden relative group"
                      initial={{ opacity: 0, y: 20 }}
                      animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-gradient-to-br from-white to-gray-100 -z-10" />
                      
                      {/* Decorative pattern */}
                      <div className="absolute inset-0 opacity-[0.03] overflow-hidden -z-5">
                        <svg width="100%" height="100%" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                          <defs>
                            <pattern id={`dots-${categoryIndex}-${memberIndex}`} width="10" height="10" patternUnits="userSpaceOnUse">
                              <circle cx="2" cy="2" r="1" fill={memberIndex % 2 === 0 ? 'var(--power-red)' : 'var(--somaiya-black)'} />
                            </pattern>
                          </defs>
                          <rect width="100%" height="100%" fill={`url(#dots-${categoryIndex}-${memberIndex})`} />
                        </svg>
                      </div>
                      
                      {/* Top colored strip with flowing animation */}
                      <div className="relative h-20 overflow-hidden">
                        <div 
                          className={`absolute inset-0 ${memberIndex % 2 === 0 ? 'bg-gradient-red' : 'bg-[var(--somaiya-black)]'}`} 
                        />
                        <motion.div 
                        ...
                        {/* Data points floating on the gradient background */}
                      ...
                      <div className="relative z-10 -mt-10 flex justify-center">
                        <motion.div 
                          className="w-20 h-20 rounded-full border-4 border-white shadow-xl flex items-center justify-center overflow-hidden"
                          whileHover={{ scale: 1.1 }}
                          transition={{ type: "spring", stiffness: 300 }}
                        >
                          <div className={`absolute inset-0 ${getAvatarColor(member.name)}`} />
                          
                          {/* Decorative rings around the avatar */}
                          ...
                          <span className="relative z-10 text-2xl font-bold">{getInitials(member.name)}</span>
                      ...
                      <div className="pt-4 pb-5 px-4 text-center">
                        ...
                        <div className="relative h-0.5 w-12 mx-auto my-2 overflow-hidden">
                          <motion.div 
                            ...
                            transition={{ 
                              duration: 0.6,
                              delay: 0.3 + (0.05 * memberIndex)
                            }}
                          />
                        </div>
                        
                        <p className="text-sm text-gray-600 mt-1 font-medium">{member.role}</p>
